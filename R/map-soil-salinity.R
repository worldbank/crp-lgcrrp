...
# salinity <- vect("mnt/source-data/Soil Salinity/Salinity.shp")
...
plots$soil_salinity <- plot_soil_salinity_regional()

# National map
plot_soil_salinity_national <- function() {
  p <- plot_layer(salinity_data,
                  yaml_key = "soil_salinity", plot_aoi = F)
  p$layers[detect_index(p$layers, \(x) inherits(x$geom, "GeomMapTile"))] <-
...
plots$soil_salinity_national <- plot_soil_salinity_national()
